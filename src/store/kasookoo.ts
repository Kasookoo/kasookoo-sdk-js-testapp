...
export const useKasookooStore = createSelectors(
    create<KasookooState>((set, get) => {
        ...
        const connectWith = async (account: LoginResult, key: string, custom: boolean) => {
            if (clientRef) return
        ...
        return {
            ...
            connect: async () => {
                const { auth, publishableKey, useCustomWindow } = get()
                if (!auth || !publishableKey.trim()) return
                await connectWith(auth, publishableKey.trim(), useCustomWindow)
            },
            disconnect: () => {
                teardown()
